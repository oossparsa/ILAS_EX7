...
public class UserSet implements Serializable {
	...
	public String toMinHashSignature() {
		String userSetString = this.userSet.toString();
		String minHashSignature = Basic.hash(0, userSetString)+"";
		for(int i =1 ; i<20 ; i++) {
			minHashSignature+="," + Basic.hash(i, userSetString);
		}
		this.minHashSignature = minHashSignature;
		return minHashSignature;
	}

	//overload method, for implementing partially computed minhash signatures 
	//should be used with i>0, with current config also j<20
	public String toMinHashSignature(int i, int j) {
		String userSetString = this.userSet.toString();
		String minHashSignature="";
		if(i==0) {
			minHashSignature= Basic.hash(0, userSetString)+"";
			for(int ii = i+1 ; ii < j+1 ; ii++) {
				minHashSignature =","+ Basic.hash(ii, userSetString);
			}
		}
		else {
